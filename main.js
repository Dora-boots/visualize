...
function createDirs() {
    for (let i = 0; i < MONTH; i++) {
        let path = Path.join(__dirname, 'public', 'results', `month${i + 2}`);
        if (!fs.existsSync(path))
            fs.mkdirSync(path);
        for (let k = 0; k < DAY; k++) {
            let day_dir_path = Path.join(__dirname, 'public', 'results', `month${i + 2}`, `day${k + 1}`);
...
function pathGenerate(month, start, end, initialPaths) {
    for (let i = start; i <= end; i++) {
        let tempPath = '';
        if (i < 10)
            tempPath = Path.join(__dirname, 'public', 'data', `20170${month}0${i}.csv`);
        else
            tempPath = Path.join(__dirname, 'public', 'data', `20170${month}${i}.csv`);
...
function process(jsonArray) {
    ...
    for (let i = 0; i < HALF; i++) {
        let type_path = Path.join(__dirname, 'public', 'results', `month${curr_month}`, `day${curr_date}`, `${i}.txt`);
        let msg_path = Path.join(__dirname, 'public', 'results', `month${curr_month}`, `day${curr_date}`, `msg${i}.txt`);
